...
usage() {
    cat <<'USAGE'
Kullanım: node/node.sh [--user www-data] [--] <node-komutu>
...
if ! command -v node >/dev/null 2>&1; then
    log_error "node komutu bulunamadı."
    exit 1
fi

log_info "node komutu çalıştırılıyor (kullanıcı: ${RUN_AS})..."

if [[ "$RUN_AS" == "root" || "$RUN_AS" == "0" ]]; then
    node "$@"
else
    run_as_user "$RUN_AS" node "$@"
fi
