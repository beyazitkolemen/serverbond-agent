...
# Vue özel pipeline fonksiyonları
# Bu dosya Vue projeleri için özel deployment adımlarını içerir

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_SCRIPT="${SCRIPT_DIR}/pipeline.sh"

# Vue özel değişkenler
FORCE_RUN_NPM=""
RUN_TESTS=false
TEST_COMMAND=""
NPM_SCRIPT="build"
...
# Vue özel kullanım bilgisi
print_vue_usage() {
    cat <<'USAGE'
Vue Pipeline Kullanımı:
  pipelines/vue.sh --repo <GIT_URL> [seçenekler]

Vue Özel Seçenekleri:
  ...
  --run-tests               npm run test:unit komutunu çalıştır
...
run_vue_steps() {
    ...
    local default_test_command="npm run test:unit"
    ...
    if [[ "${AUTO_SETUP_NGINX}" == true ]]; then
        setup_nginx_for_vue "${release_dir}"
    fi
...
setup_nginx_for_vue() {
    ...
    log_info "Vue için Nginx site kurulumu yapılıyor..."
    ...
    # Vue build dizinini bul
    local build_dir="${release_dir}/dist"
    if [[ ! -d "${build_dir}" ]]; then
        build_dir="${release_dir}/build"
        if [[ ! -d "${build_dir}" ]]; then
            build_dir="${release_dir}"
            log_warning "Vue build dizini bulunamadı, proje kök dizini kullanılıyor"
        fi
    fi
    ...
    if [[ -x "${add_site_script}" ]]; then
        ...
        if ! "${add_site_script}" "${site_args[@]}"; then
            log_error "Vue Nginx site oluşturma başarısız"
            return 1
        fi
        log_success "Vue Nginx site oluşturuldu: ${NGINX_DOMAIN}"
    else
...
vue_callback() {
    ...
    case "${action}" in
        ...
        "run_steps")
            ...
            ;;
        *)
            log_error "Bilinmeyen Vue callback action: ${action}"
            return 1
            ;;
    esac
}
...
main() {
    ...
    # Vue özel argümanları parse et
    if ! parse_vue_args "${vue_args[@]}"; then
        log_error "Vue argüman parsing başarısız"
        exit 1
    fi
    
    # Ortak pipeline'ı çağır
    source "${PIPELINE_SCRIPT}"
    run_common_deployment "vue" "vue_callback" "${common_args[@]}"
}
