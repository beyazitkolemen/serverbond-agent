...
usage() {
    cat <<'USAGE'
Kullanım: user/ssh_key_add.sh --username deploy (--key "ssh-rsa AAA..." | --key-file /path/key.pub)
USAGE
}
...
if grep -Fxq "${KEY_CONTENT}" "${AUTH_KEYS}"; then
    log_warning "Bu anahtar zaten mevcut."
    exit 0
fi

echo "${KEY_CONTENT}" >> "${AUTH_KEYS}"
log_success "SSH anahtarı eklendi: ${USERNAME}"
